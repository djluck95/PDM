...
public class AddClientFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final View v=inflater.inflate(R.layout.fragment_add_client, container, false);
        final EditText FirstName=(EditText) v.findViewById(R.id.ClientFirstName);
        final EditText LastName=(EditText) v.findViewById(R.id.ClientLastName);
                    ...
                    new Thread() {
                        public void run() {
                            try {
                                ...
                                int result = dbController.add_client(FirstName.getText().toString(),
                                ...
                                switch(result){
                                    case 1:
                                        getActivity().runOnUiThread(new Runnable() {
                                            public void run() {
                                                Toast.makeText(getActivity(), "Client successfully added!", Toast.LENGTH_LONG).show();
                                            }
                                        });
                                        break;
                                    case 0:
                                        getActivity().runOnUiThread(new Runnable() {
                                            public void run() {
                                                Toast.makeText(getActivity(), "Client already exist!", Toast.LENGTH_LONG).show();
                                            }
                                        });
                                        break;
                                    case -1:
